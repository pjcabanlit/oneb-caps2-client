...
import RegisteredTable from '../../../../../components/Tables/ResidentInformation/RegisteredTable'
...
const Registered = () => {
    ...
    useEffect(() => {
        async function fetchLoading() {
            setIsLoading(true);
            await Axios.get("http://localhost:3001/RegisteredInformation").then((response) => {
    ...
    return (
        <div>
            <div className="resident_information_tbl">
                ...
                <center>
                    <RegisteredTable data={search(registeredList)} />
                </center>
            </div>
        </div>
    )
