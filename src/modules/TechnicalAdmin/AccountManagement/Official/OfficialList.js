...
import BarangayOfficialsTable from "../../../../../src/components/Tables/BarangayOfficial/BarangayOfficialsTable";
...
const OfficialList = () => {
    ...
    useEffect(() => { async function fetchLoading() { setIsLoading(true); await Axios.get("http://localhost:3001/GetBarangayOfficials").then((response) => { setOfficialList(response.data); setIsLoading(false); }); return () => { }; } fetchLoading() }, []);
    return (
        <main>
            <div className="certificate_request"> <div className="loading_container">{isLoading && <img src={loading} alt={loading} className="loading_icon" />}</div><div className="process_request_container"> <div className="resident_information_tbl"> <div className="resident_tbl_header"> <div className="search_resident"> <i className="fa fa-search"></i> <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search Name" /> </div></div><center> <BarangayOfficialsTable data={search(official_list)} /> </center> </div></div></div>
        </main>
    );
}
