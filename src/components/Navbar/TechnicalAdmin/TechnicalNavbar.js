...
import '../../../modules/BarangayStaff/AdminHome/AdminHome.css';
...
const TechnicalNavbar = ({ openSidebar }) => {
    ...
    const [authState, setAuthState] = useState({ username: "", id: 0, status: false }), [accountUsername, setUsername] = useState(""), [role, setRole] = useState(""), history = useHistory(), [setActive, setActiveState] = useState(""), menuToggle = () => { setActiveState(setActive === "" ? "activate" : ""); }
    ...
    useEffect(() => { Axios.get("http://localhost:3001/login").then((response) => { if (response.data.loggedIn === true) { setUsername(response.data.user[0].username); setRole(response.data.user[0].role); setAccountId(response.data.user[0].account_id) } }); }, []);
    const logout = async () => {
